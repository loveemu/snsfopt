...
SPCFile * SPCFile::Load(const std::string& filename)
{
	...
	// read whole file
	uint8_t * spc_data = new uint8_t[spc_size];
	if (fread(spc_data, 1, spc_size, fp) != spc_size) {
		delete[] spc_data;
		fclose(fp);
		return NULL;
	}
	...
	SPCFile * spc = SPCFile::Load(spc_data, spc_size);

	delete[] spc_data;
	return spc;
}

SPCFile * SPCFile::Load(const uint8_t * spc_data, size_t spc_size)
{
	char * endptr;

	// signature
	if (memcmp(spc_data, SPC_SIGNATURE_HEAD, strlen(SPC_SIGNATURE_HEAD)) != 0 ||
		spc_data[0x21] != 0x1a || spc_data[0x22] != 0x1a) {
		return NULL;
	}
	...
	// SPC700 registers
	spc->regs.pc = spc_data[0x25] | (spc_data[0x26] << 8);
	spc->regs.a = spc_data[0x27];
	spc->regs.x = spc_data[0x28];
	spc->regs.y = spc_data[0x29];
	spc->regs.psw = spc_data[0x2a];
	spc->regs.sp = spc_data[0x2b];

	// RAM
	memcpy(spc->ram, &spc_data[0x100], 0x10000);

	// DSP registers
	memcpy(spc->dsp, &spc_data[0x10100], 0x80);

	// Extra RAM
	memcpy(spc->extra_ram, &spc_data[0x101c0], 0x40);

	// Parse [ID666](http://vspcplay.raphnet.net/spc_file_format.txt) if available.
	spc->tags.clear();
	if (spc_data[0x23] == 0x1a) {
		char s[256];
		uint32_t u;

		memcpy(s, &spc_data[0x2e], 32);
		s[32] = '\0';
		spc->SetStringTag(XID6_SONG_NAME, s);

		memcpy(s, &spc_data[0x4e], 32);
		s[32] = '\0';
		spc->SetStringTag(XID6_GAME_NAME, s);

		memcpy(s, &spc_data[0x6e], 16);
		s[16] = '\0';
		spc->SetStringTag(XID6_DUMPER_NAME, s);

		memcpy(s, &spc_data[0x7e], 32);
		s[32] = '\0';
		spc->SetStringTag(XID6_COMMENT, s);

		bool has_id666_song_length = false;
		if (spc_data[0xd2] < 0x30) {
			// binary format
			u = spc_data[0x9e] | (spc_data[0x9f] << 8) | (spc_data[0xa0] << 16) | (spc_data[0xa1] << 24);
			if (u != 0) {
				spc->SetIntegerTag(XID6_DUMPED_DATE, u, 4);
			}

			u = spc_data[0xa9] | (spc_data[0xaa] << 8) | (spc_data[0xab] << 16);
			if (u != 0) {
				spc->SetIntegerTag(XID6_INTRO_LENGTH, XID6TicksToMilliSeconds(u) / 1000, 4);
				has_id666_song_length = true;
			}

			u = spc_data[0xac] | (spc_data[0xad] << 8) | (spc_data[0xae] << 16) | (spc_data[0xaf] << 24);
			if (has_id666_song_length || u != 0) {
				spc->SetIntegerTag(XID6_FADE_LENGTH, XID6TicksToMilliSeconds(u), 4);
			}

			memcpy(s, &spc_data[0xb0], 32);
			s[32] = '\0';
			spc->SetStringTag(XID6_ARTIST_NAME, s);

			// [0xd0] Default channel disables (0 = enable, 1 = disable)

			spc->SetIntegerTag(XID6_EMULATOR, spc_data[0xd1], 1);
		}
		else {
			// text format
			memcpy(s, &spc_data[0x9e], 11);
			s[11] = '\0';
			if (strcmp(s, "") != 0) {
				int year;
				int month;
			...
			memcpy(s, &spc_data[0xb1], 32);
			s[32] = '\0';
			spc->SetStringTag(XID6_ARTIST_NAME, s);

			// [0xd1] Default channel disables (0 = enable, 1 = disable)

			memcpy(s, &spc_data[0xd2], 1);
			s[1] = '\0';
			if (strcmp(s, "") != 0) {
				u = strtoul(s, &endptr, 10);
				if (*endptr == '\0') {
	...
	if (spc_size > xid6_offset + 8 && memcmp(&spc_data[xid6_offset], "xid6", 4) == 0) {
		// get xid6 chunk size
		uint32_t xid6_whole_size = spc_data[xid6_offset + 4] | (spc_data[xid6_offset + 5] << 8) | (spc_data[xid6_offset + 6] << 16) | (spc_data[xid6_offset + 7] << 24);
		...
		while (xid6_offset + 4 <= xid6_end_offset) {
			XID6ItemId xid6_id = (XID6ItemId)spc_data[xid6_offset];
			XID6TypeId xid6_type = (XID6TypeId)spc_data[xid6_offset + 1];
			uint16_t xid6_length = spc_data[xid6_offset + 2] | (spc_data[xid6_offset + 3] << 8);

			if (xid6_type == XID6_TYPE_LENGTH) {
				spc->SetTagValue(xid6_id, xid6_type, &spc_data[xid6_offset + 2], 2);
				xid6_offset += 4;
			}
			else {
				xid6_offset += 4;
				...
				if (xid6_offset + xid6_length <= xid6_end_offset) {
					spc->SetTagValue(xid6_id, xid6_type, &spc_data[xid6_offset], xid6_length);
				}
